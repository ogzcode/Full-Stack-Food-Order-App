<template>
    <button @click="$emit('onOpenCommentDialog')" class="flex items-center gap-1">
        <template v-for="(star, i) in 5" :key="i">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-star-fill"
                :class="getRatingColor(props.productRating, i)" viewBox="0 0 16 16">
...
<script setup>
const emits = defineEmits(["onOpenCommentDialog"]);
