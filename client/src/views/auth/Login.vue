<template>
    <div class="flex justify-center items-center min-h-screen bg-red-50">
        <ErrorAlert :errors="errors" />
        <div class="max-[425px]:w-full xl:w-1/2 md:w-2/3 h-[520px] flex bg-white rounded-lg overflow-hidden shadow-lg">
            <div class="max-[768px]:hidden relative flex-1 z-0 flex justify-start items-center flex-col pt-4">
                <h1 class="text-white text-4xl font-black mb-4 text-center">Login to your account!</h1>
                <p class="text-center w-2/3 text-white font-semibold">Welcome to our restaurant! Please login to your
                    account using the information below.</p>
                <img class="absolute top-0 left-0 w-full h-full object-cover opacity-90 -z-10"
                    src="../../assets/image/login.jpg" alt="" />
            </div>
            <div class="flex-1 flex justify-center items-center">
                <form class="lg:w-2/3" @submit.prevent="onSubmit">
                    <h1 class="text-4xl text-center font-semibold text-slate-800">Login</h1>
                    ...
                    <p class="text-sm text-center text-gray-500">Don't have an account ? <router-link
                            class="text-rose-500 font-medium" :to="{ name: 'Register' }">Register</router-link></p>
                    <button
                        class="block mx-auto mt-6 bg-rose-500 px-8 py-2 text-white font-medium shadow-lg shadow-rose-500/50">Login</button>
                </form>
            </div>
        </div>
    </div>
...
const inputStyle = computed(() => "border border-gray-400 outline-0 w-full px-2 py-1 rounded text-gray-700 font-medium");
...
const onSubmit = () => {
    store.login({ password: password.value, username: username.value });
    password.value = '';
    username.value = '';
}
